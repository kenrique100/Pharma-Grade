...
export default function AdminUsersPage() {
  return (
    <div>
      <h1 className="text-2xl font-black text-gray-900 dark:text-white mb-8">Manage <span className="text-red-600">Users</span></h1>
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id} className="border-b border-gray-100 dark:border-gray-700/50 text-sm">
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${user.role === "admin" ? "bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-400" : "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400"}`}>{user.role}</span>
                  </td>
                  <td className="p-4 text-gray-600 dark:text-gray-300">{user.orders}</td>
                  <td className="p-4 text-gray-500 dark:text-gray-400">{user.joined}</td>
                  <td className="p-4">
                    <div className="flex gap-2">
                      ...
                      <button className="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 text-xs font-medium">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
