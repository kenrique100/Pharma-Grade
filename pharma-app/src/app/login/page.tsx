  ...
  const handleCredentials = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        setStep("otp");
        setLoading(false);
        toast.success(`OTP sent to ${email} (demo: ${code})`);
      }
    } catch {
          ...
          {step === "credentials" ? (
            ...
            <form onSubmit={handleOtp} className="space-y-5">
              <div className="text-center mb-4">
                ...
                <p className="text-gray-600 dark:text-gray-400 text-sm">We sent a 6-digit code to <strong className="text-gray-900 dark:text-white">{email}</strong></p>
              </div>
              {error && <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 px-4 py-3 rounded-lg text-sm">{error}</div>}
              <div>
                <label className="block text-gray-700 dark:text-gray-300 text-sm font-medium mb-2">OTP Code</label>
              ...
              <button type="submit" className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-lg transition-colors">Verify & Sign In</button>
